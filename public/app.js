...
const ElectronApp = {
  ...
  async checkSimulatorMode() {
    if (!this.isElectron()) return false;
    const settings = await this.getSettings();
    return settings.dataSource === 'simulator';
  },
  
  async showSimulatorBanner() {
    const isSimulator = await this.checkSimulatorMode();
...
// Show simulator banner if in Electron
if (ElectronApp.isElectron()) {
  document.addEventListener('DOMContentLoaded', () => {
    ElectronApp.showSimulatorBanner();
  });
}
    ...
    try {
      ...
      try {
        ...
        if (response.status === 429) {
          return { ok: false, error: 'Too many attempts — please wait a few minutes and try again', status: 429 };
        }
        return { ok: false, error: 'Server error — please try again', status: response.status };
      }
...
// Connect to real Apex (from simulator mode)
async function connectApex() {
  if (ElectronApp.isElectron()) {
    // TODO: Open settings modal to configure Apex connection
    alert('Apex connection setup coming soon! For now, change data source in Settings.');
  } else {
    // Web version: redirect to onboarding
...
// Make connectApex global for onclick handlers
window.connectApex = connectApex;
